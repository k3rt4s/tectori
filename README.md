...
- `scripts/check_permissions_hosts.py` requires the hosts named in the
  network egress section of `PERMISSIONS.md` and the hosts declared in
  `site/content/site.json` to be the same list in both directions. The
  manifest is what a buyer's firewall allowlist gets built from, and the
  config is what the tree is actually checked against, so a host added to one
  and not the other is either an outbound call nothing verifies or a blocked
...
- `scripts/check_class_names.py` requires every class name in `docs/` to have
  a rule in `site/static/styles.css`. A class the stylesheet never defines
  renders as nothing: the page is still valid, still reachable, still builds
...
declared value reaches a tree that passes all twenty-three checks, carries the
...
6. Rebuild in place with `python scripts/build_site.py --out docs`, then run
   ...
   twenty-three checks of its own that read the built tree as a site rather
