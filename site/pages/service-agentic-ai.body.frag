      <section class="page-hero" aria-labelledby="page-title">
        <div class="page-hero-inner">
          <p class="eyebrow">Service line 06</p>
          <h1 id="page-title">AI automation that answers the same questions as any material system.</h1>
          <p class="page-lead">
            The work delivers governed AI automation with approval gates, monitoring, decision records, and human accountability. It is built to survive the same exam questions as any material system.
          </p>
        </div>
      </section>

      <section class="content-section split-section" aria-labelledby="who-title">
        <div class="section-heading">
          <p class="eyebrow">Who this is for</p>
          <h2 id="who-title">For organizations putting AI into work that matters.</h2>
        </div>
        <div class="prose">
          <p>
            AI automation is easy to demonstrate and hard to account for. The questions that follow a demonstration are about who approved the action, what it did, how it is monitored, and how it is stopped.
          </p>
          <p>
            This work suits organizations that want automation in production and need it to hold up to the same review as any other material system. Programs align with ISO/IEC 42001 and the NIST AI Risk Management Framework.
          </p>
        </div>
      </section>

      <section class="soft-band" aria-labelledby="includes-title">
        <div class="split-section section">
          <div class="section-heading">
            <p class="eyebrow">What the work includes</p>
            <h2 id="includes-title">Automation that moves, accountability that stays human.</h2>
          </div>
          <ul class="plain-list">
            <li>AI and agent inventory</li>
            <li>Approval gates before an agent takes an action that matters</li>
            <li>Monitoring and alerting on agent behavior</li>
            <li>Verification and rollback patterns</li>
            <li>Decision records naming the accountable human</li>
            <li>Operating documentation for the people who run it</li>
          </ul>
        </div>
      </section>

      <section class="content-section" aria-labelledby="deliverables-title">
        <div class="section-heading">
          <p class="eyebrow">Deliverables</p>
          <h2 id="deliverables-title">What you receive.</h2>
        </div>
        <div class="practice-grid">
          <article class="practice-item">
            <h3>AI and agent inventories</h3>
            <p>What is running, what it can reach, what it is allowed to do, and who owns it.</p>
          </article>
          <article class="practice-item">
            <h3>Approval workflows</h3>
            <p>Defined points where a human decides, with the decision recorded.</p>
          </article>
          <article class="practice-item">
            <h3>Verification and rollback patterns</h3>
            <p>How the output is checked before it is trusted, and how an action is undone.</p>
          </article>
          <article class="practice-item">
            <h3>Operating documentation</h3>
            <p>How the system runs, how it fails, and how it is stopped.</p>
          </article>
        </div>
      </section>

      <section class="content-section split-section" aria-labelledby="readiness-title">
        <div class="section-heading">
          <p class="eyebrow">How it supports audit readiness</p>
          <h2 id="readiness-title">The exam answer is documentation, not a demonstration.</h2>
        </div>
        <div class="prose">
          <p>
            AI work ships with approvals, records, and a stop path, so the exam answer is documentation instead of a demonstration. The governing questions are the ones already asked about any system that can change data or spend money.
          </p>
          <p>
            The governance model is described on <a class="text-link" href="/ai-governance">AI governance</a>.
          </p>
        </div>
      </section>

      <section class="dark-band" aria-labelledby="scope-title">
        <div class="split-section section">
          <div class="section-heading">
            <p class="eyebrow">What this is not</p>
            <h2 id="scope-title">What this service line does not cover.</h2>
          </div>
          <ul class="plain-list">
            <li>Model training, fine tuning, or data science work.</li>
            <li>An AI product or a platform license.</li>
            <li>Autonomy without a stop path. Every agent has one.</li>
          </ul>
        </div>
      </section>

      <section class="cta-band">
        <div class="cta-inner">
          <h2>Use AI at the speed your accountability model supports.</h2>
          <a class="button primary" href="/contact">Discuss agentic AI</a>
        </div>
      </section>

